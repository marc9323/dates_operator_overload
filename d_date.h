...
public:
    ...
    // prototype for friend function to handle input and output streaming
    friend std::ostream& operator<<(std::ostream& outs, const date& source);
    friend std::istream& operator>>(std::istream& ins, date& target);
...
ostream& operator<<(std::ostream& outs, const date& source) {
    outs << source.getDay() << "/" << source.getMonth() << "/" << source.getYear();
    return outs;
}

istream& operator>>(std::istream& ins, date& target) {
...
// overloaded postfix operator - increments the date by a single day
// utilizing date class help function incrementDate(int)
...
{
    // if the years are equal go on to compare the months
    // if the months are equal go on to compare the days
    ...
    else if (this->year == d.year)
    {
        ...
        else if (this->month > d.month)
        {
            ...
            // the months are the same, compare the days
